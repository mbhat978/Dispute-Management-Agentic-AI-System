...
This specialist agent gathers evidence using a dynamic ReAct-style tool plan 
with LLM guidance and rule-based fallback. Restored from original investigator.py
with full LLM planning, 2-stage execution, and parallel tool execution.
"""
...
def data_retrieval_node(state: DisputeState) -> Dict[str, Any]:
    """
    Data Retrieval Agent: Gathers evidence using LLM-powered planning and parallel execution.
    
    This agent uses the exact logic from the original investigator:
    1. LLM-powered investigation planning with reasoning
    2. Rule-based fallback for robustness
    3. 2-stage execution: transaction_details first, then parallel execution
    4. Merchant name extraction and injection for dependent tools
    ...
    try:
        ...
        # Build investigation plan using LLM planner with fallback
        ...
        logger.info(
            f"[DATA RETRIEVAL AGENT] plan_created | planner={planner_mode} | "
            f"steps={[step['tool'] for step in plan]}"
        )
        audit_trail.append(
            f"Data Retrieval Agent THOUGHT: Built investigation plan using {planner_mode}. "
            f"Planned steps: {[step['tool'] for step in plan]}"
        )
        
        # STAGE 1: Execute get_transaction_details synchronously first
        merchant_name = ""
        ...
        if transaction_details_step:
            tool_name = transaction_details_step["tool"]
            tool_input = transaction_details_step["input"]
            data_key = transaction_details_step["data_key"]
            
            logger.info(f"[DATA RETRIEVAL AGENT] STAGE 1: Executing {tool_name}")
            audit_trail.append(
            ...
            try:
                ...
                # Extract merchant_name for dependent tools
                if isinstance(observation, dict):
                    merchant_name = observation.get("merchant_name", "")
                
                logger.info(
                    f"[DATA RETRIEVAL AGENT] Observation: {tool_name} returned data | "
        ...
        # STAGE 2: Execute remaining tools in parallel with merchant_name injection
        if remaining_steps:
            logger.info(f"[DATA RETRIEVAL AGENT] STAGE 2: Executing {len(remaining_steps)} tools in parallel")
            
            # Inject merchant_name into tools that need it
            for step in remaining_steps:
                tool_input = step["input"]
                
                # Tools that require merchant_name
                if step["tool"] in [
                    ...
                    # Inject merchant_name if it's empty or a placeholder
                    current_merchant = tool_input.get("merchant_name", "")
                    if not current_merchant or current_merchant in ["", "<merchant_name>"]:
                        tool_input["merchant_name"] = merchant_name
                        logger.debug(
                            f"[DATA RETRIEVAL AGENT] Injected merchant_name='{merchant_name}' "
                            f"into {step['tool']}"
                        )
                
                # Inject merchant_name into analyze_receipt_evidence
                if step["tool"] == "analyze_receipt_evidence":
                    current_merchant = tool_input.get("expected_merchant", "")
                    if not current_merchant or current_merchant in ["", "<merchant_name>"]:
                        tool_input["expected_merchant"] = merchant_name
                        logger.debug(
                            f"[DATA RETRIEVAL AGENT] Injected expected_merchant='{merchant_name}' "
                            f"into analyze_receipt_evidence"
                        )
            
            # Execute tools in parallel using ThreadPoolExecutor
            with ThreadPoolExecutor(max_workers=5) as executor:
                future_to_step = {
                    executor.submit(_execute_tool, step["tool"], step["input"]): step
                    for step in remaining_steps
                }
...
def _build_investigation_plan(
    ...
    try:
        # Initialize LLM with gpt-4o and temperature=0 (exact from original)
        import httpx
        ...
        # Enhanced prompt with reasoning requirements (exact from original)
...
Create an intelligent investigation plan with reasoning:""")
        ...
        logger.debug(f"[DATA RETRIEVAL AGENT] action=invoke_llm_planner | model=gpt-4o")
        ...
        logger.info(f"[DATA RETRIEVAL AGENT] llm_plan_reasoning={reasoning[:100]}...")
        logger.info(f"[DATA RETRIEVAL AGENT] llm_planned_steps={len(steps)}")
...
def _rule_based_plan(
    ...
    """
    Rule-based fallback planner (exact from original investigator.py).
    """
...
def _sanitize_plan(
    ...
    """
    Sanitize and validate LLM plan (exact from original investigator.py).
    """
    allowed = {
...
def _execute_tool(tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, Any]:
    """
    Execute a single tool and return its result (exact from original investigator.py).
    """
